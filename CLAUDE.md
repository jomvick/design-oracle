...
Ce projet expose un serveur MCP (Model Context Protocol) pour connecter des agents IA et IDE (opencode, Claude Code, Cursor, Codex, etc.). Le serveur MCP wraps l'API d'analyse en outils et ressources.
...
**Claude Code** → dans `~/.claude/settings.json` ou `claude.json` :
```json
{
  "mcpServers": {
    "design-oracle": {
